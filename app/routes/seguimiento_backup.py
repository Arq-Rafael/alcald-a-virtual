...
def _load_plan_excel():
    """Carga PLAN_DESARROLLO (198 metas únicas) + REGISTRO_AVANCES (datos por año 2024/2025)."""
    global _plan_cache
    ...
    try:
        ...
        # 1. Cargar PLAN_DESARROLLO (198 metas únicas - base maestra)
        if 'PLAN_DESARROLLO' not in excel_file.sheet_names:
            logger.error(f"Hoja 'PLAN_DESARROLLO' no encontrada")
            return None
        
        plan_df = pd.read_excel(file_path, sheet_name='PLAN_DESARROLLO', header=0)
        logger.info(f"Cargadas {len(plan_df)} metas del Plan de Desarrollo")
        
        # 2. Cargar REGISTRO_AVANCES (datos por año: 2024 y 2025)
        if 'REGISTRO_AVANCES' not in excel_file.sheet_names:
            logger.error(f"Hoja 'REGISTRO_AVANCES' no encontrada")
            return None
